...
function initGiffer() {
  ...
  Gifffer = function Gifffer(options) {
    var images,
        i = 0,
        gifs = [];
  ...
  function createContainer(w, h, el, altText, opts) {
    ...
    con.setAttribute('aria-hidden', 'true'); // creating play button
    ...
    play.appendChild(trngl); // create alt text if available
  ...
  function process(el, gifs, options) {
    ...
    el.style.display = 'block'; // creating the canvas

    c = document.createElement('canvas');
    isC = !!(c.getContext && c.getContext('2d'));
    if (w && h && isC) cc = createContainer(w, h, el, altText, options); // waiting for image load

    el.onload = function () {
      if (!isC) return;
      w = w || el.width;
      h = h || el.height; // creating the container

      if (!cc) cc = createContainer(w, h, el, altText, options);
      con = cc.c;
      play = cc.p;
      dims = calculatePercentageDim(con, w, h, el.width, el.height); // add the container to the gif arraylist

      gifs.push(con); // listening for image click
      ...
      con.appendChild(c); // setting the actual image size
...
initGiffer();
//# sourceMappingURL=gifffer.js.map
